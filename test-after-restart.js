// Teste após reiniciar o servidor
...
async function testAfterRestart() {
  ...
  try {
    ...
    if (supabaseUrl) {
      console.log('✅ VITE_SUPABASE_URL encontrada:', supabaseUrl);
    } else {
      console.log('❌ VITE_SUPABASE_URL não encontrada');
    }
    
    if (supabaseKey) {
      console.log('✅ VITE_SUPABASE_PUBLISHABLE_KEY encontrada:', supabaseKey.substring(0, 20) + '...');
    } else {
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.log('1. ❌ Variáveis de ambiente não encontradas');
      console.log('2. 🔧 Verificar se o arquivo .env está na raiz do projeto');
      console.log('3. 🔧 Verificar se o servidor foi reiniciado');
      console.log('4. 🔧 Verificar se há erros no console');
