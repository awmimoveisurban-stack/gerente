...
-- =====================================================================================
-- 3. CRIAR USUÁRIO COM SENHA CORRETA
-- =====================================================================================
...
BEGIN
    ...
    -- Criar usuário corretor com senha corretamente criptografada
    INSERT INTO auth.users (
        ...
        crypt('12345678', gen_salt('bf')), -- Senha corretamente criptografada
        NOW(),
        NOW(),
        NOW(),
    ...
    RAISE NOTICE '✅ Usuário corretor criado com senha correta!';
END $$;
...
SELECT 
    ...
    END as status_email,
    CASE 
        WHEN encrypted_password IS NOT NULL THEN '✅ Senha existe'
        ELSE '❌ Senha não existe'
    END as status_senha
FROM auth.users 
WHERE email = 'corretor@imobiliaria.com';
...
SELECT 
    ...
    END as email,
    CASE 
        WHEN EXISTS (SELECT 1 FROM auth.users WHERE email = 'corretor@imobiliaria.com' AND encrypted_password IS NOT NULL) 
        THEN '✅ Senha existe'
        ELSE '❌ Senha não existe'
    END as senha,
    CASE 
...
SELECT 'Status: Credenciais válidas' as status;

SELECT '🎉 CREDENCIAIS CORRIGIDAS COM SUCESSO!' as resultado;
