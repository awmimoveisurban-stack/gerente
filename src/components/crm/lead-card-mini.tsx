...
export function LeadCardMini({ lead, onViewDetails, onCall, onEmail, onScheduleVisit }: LeadCardMiniProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <Card
      ...
      className={`cursor-grab active:cursor-grabbing hover:shadow-sm transition-all duration-200 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50 ${
        isDragging ? 'shadow-md ring-2 ring-primary scale-105' : 'hover:scale-[1.01]'
      }`}
    >
      <CardContent className="p-2">
        {/* Header - Nome e Menu em linha */}
        <div className="flex items-center justify-between mb-1">
          <h3 className="font-semibold text-gray-900 dark:text-white truncate text-sm flex-1">{lead.nome}</h3>
          <DropdownMenu>
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            {lead.valor_interesse && (
              <Badge variant="outline" className="font-mono bg-green-50 dark:bg-green-950/20 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800 px-1 py-0.5 text-xs">
