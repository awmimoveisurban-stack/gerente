...
import { Filter, RefreshCcw, Users, Target } from 'lucide-react';

interface TodosLeadsFiltersProps {
  statusFilter: string;
  corretorFilter: string;
  origemFilter?: string; // ✅ MELHORIA #5: Filtro por origem
  onStatusChange: (status: string) => void;
  onCorretorChange: (corretor: string) => void;
  onOrigemChange?: (origem: string) => void; // ✅ MELHORIA #5
  onResetFilters: () => void;
  getStatusCount: (status: string) => number;
  getCorretorCount: (corretor: string) => number;
  getOrigemCount?: (origem: string) => number; // ✅ MELHORIA #5
  corretores: string[];
}
...
export const TodosLeadsFilters = memo(function TodosLeadsFilters({
  statusFilter,
  corretorFilter,
  origemFilter = 'todos', // ✅ MELHORIA #5
  onStatusChange,
  onCorretorChange,
  onOrigemChange, // ✅ MELHORIA #5
  onResetFilters,
  getStatusCount,
  getCorretorCount,
  getOrigemCount, // ✅ MELHORIA #5
  corretores,
    ...
    <Card className='bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50 shadow-lg'>
      ...
      <CardContent className='p-6'>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
          ...
          {/* ✅ MELHORIA #5: Filtro por Origem */}
          {onOrigemChange && getOrigemCount && (
            <div className='space-y-3'>
              <div className='flex items-center gap-2'>
              ...
              <Select value={origemFilter} onValueChange={onOrigemChange}>
                ...
                <SelectContent>
                  <SelectItem value='todos'>
                    🌐 Todas ({getOrigemCount('todos')})
                  </SelectItem>
                  <SelectItem value='whatsapp'>
                    📱 WhatsApp ({getOrigemCount('whatsapp')})
                  </SelectItem>
                  <SelectItem value='site'>
                    🌐 Site ({getOrigemCount('site')})
                  </SelectItem>
                  <SelectItem value='indicacao'>
                    👥 Indicação ({getOrigemCount('indicacao')})
                  </SelectItem>
                  <SelectItem value='redes_sociais'>
                    📸 Redes Sociais ({getOrigemCount('redes_sociais')})
                  </SelectItem>
                  <SelectItem value='telefone'>
                    ☎️ Telefone ({getOrigemCount('telefone')})
                  </SelectItem>
                  <SelectItem value='outros'>
                    ❓ Outros ({getOrigemCount('outros')})
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
